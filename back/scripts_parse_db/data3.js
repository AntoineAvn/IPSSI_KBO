...
let batchSize = 10000; // Taille des lots pour l'insertion
...
const processBatch = async (batch) => {
  ...
  try {
    const result = await establishmentCollection.bulkWrite(bulkOperations);
    console.log(`Batch processed: ${result.modifiedCount} establishments updated.`);
  } catch (error) {
...
const run = async () => {
  try {
    ...
    // Ajout de l'index sur establishmentNumber pour optimiser les recherches
    await db.collection('establishment').createIndex({ establishmentNumber: 1 });
    console.log('Index on establishmentNumber created.');
